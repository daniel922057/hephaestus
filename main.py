...
def main():
    ...
    # 持仓逻辑
    if signal.trend == 1:
        okx_client.open_position(symbol=symbol,direction='long',amount=200,leverage=5)
        # 开多网格
        okx_client.open_grid_if_not_exist(symbol=symbol,direction='long',amount=300,leverage=5,atr=atr_value)
    elif signal.trend == -1:
        okx_client.open_position(symbol=symbol,direction='short',amount=200,leverage=5)
        # 开多网格
        okx_client.open_grid_if_not_exist(symbol=symbol,direction='short',amount=300,leverage=5,atr=atr_value)
